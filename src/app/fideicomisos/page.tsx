    ...
    <AppShell>
      <div className="p-6 md:p-10 max-w-7xl">
        <PageHeader
          kicker="Grupo ADI"
          title="Fideicomisos"
          description={`${fideicomisos.length} fideicomiso${fideicomisos.length === 1 ? "" : "s"} en el sistema. El alta de nuevos se hace por migración; acá se editan los existentes.`}
        />
              ...
              {fideicomisos.map((f) => {
                ...
                return (
                  <tr key={f.id || f.slug} className="hover:bg-slate-50 transition">
                    ...
                    <td className="px-4 py-3 text-sm text-slate-600">{f.inicio_actividad || <span className="text-slate-400">—</span>}</td>
