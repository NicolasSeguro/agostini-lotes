...
export function DetalleVentaActions({
  ...
  async function handleAutorizar() {
    if (!confirm("Â¿ConfirmÃ¡s la autorizaciÃ³n de esta venta?")) return;
      ...
      <div className="bg-gradient-to-br from-orange-50 to-amber-50 border border-brand-200 rounded-xl p-5">
        {venta.estado === "EN_CARGA" && (
          <div className="space-y-3">
            <div className="flex flex-wrap gap-3 items-center">
              ...
              >
                <XCircle size={16} />
                {venta.anticipo_cobrado > 0 ? "Solicitar anulaciÃ³n con reintegro" : "Anular venta"}
              </button>
            </div>
            {venta.anticipo_cobrado > 0 && (
              <div className="text-xs text-slate-600 bg-slate-50 border border-slate-200 rounded p-2">
                ...
                Si seguÃ­s editando y cerrÃ¡s, la venta vuelve al flujo normal. Si la anulÃ¡s, pasa a Caja para el reintegro.
              </div>
            )}
          </div>
        )}
          ...
          <div className="space-y-3">
            ...
            {venta.anticipo > 0 && (
              <div className="text-xs text-slate-600">
                Cobrado: {formatMoney(venta.anticipo_cobrado)} de {formatMoney(venta.anticipo)} Â· 
        ...
        {venta.estado === "CERRADA_CONFIRMADA" && (
          <div className="flex flex-wrap gap-3 items-center">
            <span className="text-sm text-slate-700 font-medium">
              Pendiente autorizaciÃ³n del Gerente Comercial
        ...
        {venta.estado === "AUTORIZADA" && (
          <div className="space-y-2">
            <div className="flex flex-wrap gap-3 items-center">
              <span className="text-sm text-slate-700 font-medium">Pendiente contabilizaciÃ³n</span>
              <button
              ...
              {venta.total_descuentos_off_books > 0 && (
                ...
                >
                  Revertir reclasificaciÃ³n ({formatMoney(venta.total_descuentos_off_books)})
                </button>
              )}
              <button
            ...
            {venta.total_descuentos_off_books > 0 && (
              <div className="text-xs text-purple-700">
                ...
                PodÃ©s revertir para volver al estado original o seguir hacia contabilizar.
              </div>
            )}
          </div>
        )}
        ...
        {(venta.estado === "RECHAZADA_COMERCIAL" || venta.estado === "RECHAZADA_CONTABILIDAD") && (
          <div className="space-y-3">
            <div className="text-sm text-red-700 font-medium inline-flex items-center gap-2">
              ...
              {" â€” Ver historial para el motivo. Â¿QuÃ© hacer?"}
            </div>
            <div className="flex flex-wrap gap-3">
              ...
              {venta.anticipo_cobrado > 0 && (
                ...
                >
                  <XCircle size={16} />
                  Solicitar anulaciÃ³n con reintegro
                </button>
              )}
              {venta.anticipo_cobrado === 0 && (
                <button
            ...
            {venta.anticipo_cobrado > 0 && (
              <div className="text-xs text-slate-600 bg-slate-50 border border-slate-200 rounded p-2">
                Anticipo cobrado vigente: <strong>{formatMoney(venta.anticipo_cobrado)}</strong>.
                Si reanudÃ¡s la venta, este monto se reutiliza. Si la anulÃ¡s, pasa a Caja para el reintegro.
              </div>
            )}
          </div>
        )}
        ...
        {venta.estado === "PENDIENTE_REINTEGRO" && (
          <div className="text-sm text-orange-700 font-medium inline-flex items-center gap-2">
            ...
            Pendiente reintegro por Caja. El vendedor ya solicitÃ³ la anulaciÃ³n.
          </div>
        )}
    ...
    <ModalBase title="Cobrar anticipo" onClose={onClose}>
      <form onSubmit={submit} className="space-y-4">
        <div className="text-xs bg-blue-50 border border-blue-200 rounded p-2">
          ...
          {" Â· "}Ya cobrado: <span className="font-medium">{formatMoney(venta.anticipo_cobrado)}</span>
          {" Â· "}Falta: <span className="font-medium text-green-700">{formatMoney(venta.anticipo_restante)}</span>
        </div>
        ...
        {requiereBanco && (
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-xs text-slate-600 mb-1 block">NÃºmero operaciÃ³n</label>
              <input
                type="text"
                value={numOp}
    ...
    <ModalBase title={`Rechazar venta (${rol === "COMERCIAL" ? "Gerente Comercial" : "Contabilidad"})`} onClose={onClose}>
      <form onSubmit={submit} className="space-y-4">
        <div className="text-xs bg-amber-50 border border-amber-200 rounded p-2">
          La venta quedarÃ¡ marcada como <strong>{rol === "COMERCIAL" ? "RECHAZADA_COMERCIAL" : "RECHAZADA_CONTABILIDAD"}</strong>.
          {venta.anticipo_cobrado > 0 && (
            <div className="mt-1">
              Las cobranzas del anticipo ({formatMoney(venta.anticipo_cobrado)}) <strong>se mantienen vivas</strong>.
              El vendedor decidirÃ¡ si corrige y reanuda (reutilizando el anticipo) o si solicita anulaciÃ³n con reintegro.
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            rows={4}
            autoFocus
            placeholder="DescribÃ­ quÃ© falla y quÃ© hay que corregir..."
            className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-500"
          />
          <div className="text-xs text-slate-500 mt-1">MÃ­nimo 3 caracteres</div>
        </div>
...
function ModalReclasificar({ tenant, venta, onClose, onCall, submitting, error }: any) {
  ...
  function submit(e: FormEvent) {
    ...
    if (!confirm(`Â¿Reclasificar ${formatMoney(montoReclas)} como descuento comercial?`)) return;
  ...
  // Preview de cÃ³mo queda la venta
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        <div>
          ...
          <div className="text-xs text-slate-500 mt-1">
            MÃ¡ximo: {formatMoney(venta.anticipo_cobrado)} (anticipo cobrado disponible)
          </div>
        </div>
        ...
        {m > 0 && m <= venta.anticipo_cobrado && (
          <div className="border border-brand-200 bg-brand-50/50 rounded-lg p-3 text-xs space-y-1">
            <div className="font-semibold text-slate-700 mb-2">Vista previa despuÃ©s de reclasificar:</div>
            ...
            <div className="text-slate-500 pt-2 border-t border-brand-200 mt-2">
              Cuotas: <strong>{formatMoney(venta.cuota_base)}</strong> Ã— {venta.cant_cuotas} (no cambian)
            </div>
          </div>
        )}
        ...
        <div>
          ...
          <textarea
            ...
            onChange={(e) => setObs(e.target.value)}
            rows={2}
            placeholder="Motivo de la reclasificaciÃ³n..."
            className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
          />
        </div>
...
function ModalContabilizar({ tenant, venta, onClose, onCall, submitting, error }: any) {
  ...
  function submit(e: FormEvent) {
    e.preventDefault();
    if (!confirm(`Â¿Contabilizar la venta? Se generarÃ¡n ${venta.cant_cuotas} cuotas y el lote quedarÃ¡ VENDIDO.`)) return;
    ...
    <ModalBase title="Contabilizar venta" onClose={onClose}>
      <form onSubmit={submit} className="space-y-4">
        <div className="text-xs bg-green-50 border border-green-200 rounded p-2 space-y-1">
          ...
          <div>â€¢ Se generan {venta.cant_cuotas} cuotas ({venta.sistema_amort})</div>
          <div>â€¢ Las cobranzas BORRADOR pasan a CONFIRMADA (emiten recibo oficial)</div>
          <div>â€¢ El lote queda VENDIDO</div>
          <div>â€¢ La venta queda CONTABILIZADA (no se puede deshacer)</div>
        </div>
        ...
        <div>
          ...
          {fechaYaPaso && (
            <div className="text-xs text-amber-700 mt-1 inline-flex items-center gap-1">
              ...
              La fecha ya pasÃ³. La primera cuota nacerÃ¡ vencida.
            </div>
          )}
        </div>
...
function ModalCorregirRechazo({ tenant, venta, onClose, onCall, submitting, error }: any) {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      // Si tildÃ³ cambiar lote â†’ ir a editar con el flag
      // Si no â†’ ir a editar normal
      ...
      setLocalError(err.message || "Error de conexiÃ³n");
      setLocalSubmitting(false);
    }
  }
    ...
    <ModalBase title="Corregir y reanudar venta" onClose={onClose}>
      <form onSubmit={submit} className="space-y-4">
        <div className="text-xs bg-blue-50 border border-blue-200 rounded p-2 space-y-1">
          ...
          <div>â€¢ La venta vuelve a <strong>EN_CARGA</strong></div>
          <div>â€¢ Se conservan las cobranzas del anticipo ({formatMoney(venta.anticipo_cobrado)})</div>
          <div>â€¢ El lote sigue RESERVADO</div>
          <div>â€¢ Vas a poder editar la venta antes de cerrarla de nuevo</div>
        </div>
        ...
        {cambiarLote && (
          <div className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded p-2">
            En la prÃ³xima pantalla podrÃ¡s elegir otro lote. El anticipo cobrado se mantiene asociado a la venta.
          </div>
        )}
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// MODAL: SOLICITAR ANULACIÃ“N CON REINTEGRO
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
function ModalSolicitarAnulacion({ tenant, venta, onClose, onCall, submitting, error }: any) {
  ...
  return (
    <ModalBase title={hayAnticipo ? "Solicitar anulaciÃ³n con reintegro" : "Anular venta"} onClose={onClose}>
      <form onSubmit={submit} className="space-y-4">
        {hayAnticipo ? (
          <div className="text-xs bg-red-50 border border-red-200 rounded p-2 space-y-1">
            ...
            <div>â€¢ La venta pasa a <strong>PENDIENTE_REINTEGRO</strong></div>
            <div>â€¢ AparecerÃ¡ en el panel de Caja para procesar el reintegro</div>
            <div>â€¢ Monto a reintegrar: <strong>{formatMoney(venta.anticipo_cobrado)}</strong></div>
            <div>â€¢ El cajero efectuarÃ¡ el egreso al cliente</div>
            <div>â€¢ Cuando se procese el reintegro, la venta quedarÃ¡ ANULADA y el lote DISPONIBLE</div>
          </div>
        ) : (
          <div className="text-xs bg-red-50 border border-red-200 rounded p-2 space-y-1">
            <div className="font-medium text-red-900">Al anular:</div>
            <div>â€¢ La venta pasa a <strong>ANULADA</strong> directamente (no hay anticipo cobrado)</div>
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            autoFocus
            placeholder="Â¿Por quÃ© se anula esta venta? El cliente lo verÃ¡ en su comprobante de reintegro."
            className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-500"
          />
          <div className="text-xs text-slate-500 mt-1">MÃ­nimo 3 caracteres</div>
        </div>
        ...
        <ModalActions 
          onClose={onClose} 
          submitting={submitting} 
          label={hayAnticipo ? "Solicitar anulaciÃ³n" : "Anular venta"} 
          labelColor="red"
    ...
    <ModalBase title="Editar venta (volver a EN_CARGA)" onClose={onClose}>
      <form onSubmit={submit} className="space-y-4">
        <div className="text-xs bg-blue-50 border border-blue-200 rounded p-2 space-y-1">
          ...
          <div>â€¢ La venta vuelve a <strong>EN_CARGA</strong></div>
          <div>â€¢ Las cobranzas del anticipo {venta.anticipo_cobrado > 0 ? `(${formatMoney(venta.anticipo_cobrado)}) ` : ""}se conservan</div>
          <div>â€¢ El lote sigue RESERVADO</div>
          <div>â€¢ Cuando vuelvas a cerrar, el anticipo cobrado se reutiliza</div>
        </div>
